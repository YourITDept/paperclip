...
describe("provisioning", () => {
  it("creates a private directory and seeds settings.json", async () => {
    const dir = await ensureVaultDir(CO, "fresh", env);
    ...
    expect(await readVaultSummary(CO, "fresh", env)).toMatchObject({
      name: "fresh",
      hasCredential: false,
      tokenSuffix: null,
    });
  ...
  it("never overwrites an existing settings.json", async () => {
    const dir = await ensureVaultDir(CO, "keepsettings", env);
    await fs.writeFile(path.join(dir, "settings.json"), '{"custom":true}');
    await ensureVaultDir(CO, "keepsettings", env);
  ...
  it("promotes a token and reports it without exposing it", async () => {
    await promoteVaultCredential(CO, "signedin", SETUP_TOKEN, env);
    const summary = await readVaultSummary(CO, "signedin", env);
    ...
    const mode = (await fs.stat(path.join(root, CO, "signedin", ".credentials.json"))).mode & 0o777;
    expect(mode).toBe(0o600);
  });

  it("rejects a token that is not token-shaped", async () => {
    for (const bad of ["", "   ", "has spaces", "short", "a".repeat(600)]) {
      await expect(promoteVaultCredential(CO, "bad", bad, env)).rejects.toThrow(
        CLAUDE_VAULT_CREDENTIAL_REJECTED,
      );
    }
  });
  ...
  it("reports a login-written credential as oauth, not setup_token", async () => {
    ...
    const dir = await ensureVaultDir(CO, "interactive", env);
    await fs.writeFile(
    ...
    expect(await readVaultSummary(CO, "interactive", env)).toMatchObject({
      hasCredential: true,
      authMode: "oauth",
      subscriptionType: "max",
    });
  ...
  it("lists only well-formed vaults and tolerates a missing root", async () => {
    await ensureVaultDir(CO, "one", env);
    await ensureVaultDir(CO, "two", env);
    await fs.mkdir(path.join(root, "Not A Vault"), { recursive: true });
    expect((await listVaults(CO, env)).map((v) => v.name)).toEqual(["one", "two"]);
    expect(await listVaults(CO, { [CLAUDE_VAULT_ROOT_ENV_KEY]: path.join(root, CO, "absent") })).toEqual([]);
  });
});
...
describe("removing a credential", () => {
  it("removes the credential and keeps the vault usable", async () => {
    ...
    await promoteVaultCredential(CO, "keeper", SETUP_TOKEN, env);
    ...
    await expect(fs.access(path.join(dir, "settings.json"))).resolves.toBeUndefined();
    expect(await readVaultSummary(CO, "keeper", env)).toMatchObject({ hasCredential: false });
  ...
  it("signing in again after a removal restores the credential", async () => {
    await promoteVaultCredential(CO, "cycle", SETUP_TOKEN, env);
  ...
  it("rejects a traversing name instead of unlinking outside the root", async () => {
    await expect(removeVaultCredential(CO, "../escape", env)).rejects.toThrow(CLAUDE_VAULT_NAME_INVALID);
  });
...
describe("deleting a vault", () => {
  it("removes the whole directory and leaves its siblings alone", async () => {
    ...
    await promoteVaultCredential(CO, "doomed", SETUP_TOKEN, env);
    await promoteVaultCredential(CO, "survivor", SETUP_TOKEN, env);
  ...
  it("refuses a traversing name rather than removing a directory outside the root", async () => {
    ...
    try {
      await expect(deleteVault(CO, "../claude-not-a-vault", env)).rejects.toThrow(
        CLAUDE_VAULT_NAME_INVALID,
      );
