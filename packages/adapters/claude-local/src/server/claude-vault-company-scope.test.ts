...
// FORK-CARRIED (CustomCodeDoc §4 change set 4). The company tier is a security
// boundary, so these assert what CANNOT happen, not only what can.
const ENV = { PAPERCLIP_CLAUDE_VAULT_ROOT: "/sysops/llm/claude" } as NodeJS.ProcessEnv;
...
describe("vault company scope", () => {
  it("nests a vault under its company id", () => {
    expect(resolveCompanyVaultDir(CO, "alice", ENV)).toBe(`/sysops/llm/claude/${CO}/alice`);
  });
  ...
  it.each([["../evil"], ["../../etc"], ["a/b"], [".."]])(
    ...
    (bad) => {
      expect(() => resolveCompanyVaultDir(CO, bad, ENV)).toThrow(CLAUDE_VAULT_NAME_INVALID);
    },
  );
