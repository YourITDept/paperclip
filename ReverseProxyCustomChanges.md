...
**Status:** Investigation complete, no code changed yet. For review.
**Owner:** cwa@youritdept.com
**Repo:** `YourITDept/paperclip` (fork of `paperclipai/paperclip`), branch `DEVLOPMENT`
**Created:** 2026-08-01

> **How to use this file.** This is a living document. The top half (Problem →
> Options → Risks) is the proposal for review. The bottom half (**Running Log**)
> is an append-only record of everything we actually do, so the work can be
> repeated, audited, or handed to someone else. Add a new dated entry to the log
> for every session; don't rewrite old entries, amend them with a follow-up line.
...
[`server/src/middleware/auth.ts:429`](server/src/middleware/auth.ts#L429) —
...
([`auth.ts:430-431`](server/src/middleware/auth.ts#L430-L431)).
...
[`server/src/middleware/auth.ts:163`](server/src/middleware/auth.ts#L163) —
`actorMiddleware()` resolves `req.actor` for every request, in this order:

1. Default actor: in `local_trusted` mode, an implicit admin board user; otherwise `{ type: "none" }` ([:166-176](server/src/middleware/auth.ts#L166-L176))
2. **If no `Authorization: Bearer` header** and mode is `authenticated`:
   a. `resolveCloudTenantActor()` — trusted headers ([:183](server/src/middleware/auth.ts#L183))
   b. Better Auth session cookie ([:195](server/src/middleware/auth.ts#L195))
...
[`app.ts:317`](server/src/app.ts#L317) *before* the Better Auth catch-all handler
at [`app.ts:318-320`](server/src/app.ts#L318-L320). The synthetic session ID is
literally `paperclip:${req.actor.source}:${req.actor.userId}`.

The React UI gates on exactly that endpoint
([`ui/src/api/auth.ts:145`](ui/src/api/auth.ts#L145), consumed across ~15 pages).
...
[`app.ts:284`](server/src/app.ts#L284). We get real client/proxy IPs for free by
...
- `PAPERCLIP_DEPLOYMENT_MODE` — `local_trusted` (default) vs `authenticated`. **The trusted-header and session paths only run in `authenticated`** ([`auth.ts:182`](server/src/middleware/auth.ts#L182)).
- `PAPERCLIP_DEPLOYMENT_EXPOSURE` — `private` / public.
- `PAPERCLIP_ALLOWED_HOSTNAMES` — feeds a hostname guard and Better Auth trusted origins ([`better-auth.ts:118`](server/src/auth/better-auth.ts#L118)). Must include the external hostname or requests through Traefik get rejected.
- `PAPERCLIP_PUBLIC_URL` / `PAPERCLIP_AUTH_BASE_URL_MODE` + `PAPERCLIP_AUTH_PUBLIC_BASE_URL` — drive secure-cookie behaviour ([`better-auth.ts:79`](server/src/auth/better-auth.ts#L79)). Behind TLS-terminating Traefik these need to reflect the *external* HTTPS URL, not `localhost:3100`.
