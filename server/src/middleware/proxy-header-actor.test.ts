...
describe("resolveProxyHeaderActor", () => {
  it("returns null when proxy auth is not enabled", async () => {
    // Stubbed explicitly rather than relying on the ambient environment: a host
    // that actually runs behind forward auth exports PAPERCLIP_PROXY_AUTH_ENABLED=true,
    // which would otherwise leak in and make this assertion fail.
